...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!action || action === "validate") {
      ...
      // Tenta 1: com o HWID real (key já vinculada)
      const attempt = async (sendHwid) => {
        const body = {
        ...
        const r = await fetch(`${PANDA_BASE}/keys/validate-account`, {
          method:  "POST",
      ...
      // Primeira tentativa: com HWID
      let data = hwid ? await attempt(true) : null;

      // Se falhou com HWID (key ainda não vinculada), tenta sem HWID
      // para deixar o Panda vincular automaticamente nesta execução
      if (!data || data.Authenticated_Status !== "Success") {
...
// Converte "MM/DD/YY HH:MM" ou "MM/DD/YYYY" → ISO string
